...
public class BuildingXmlReader {
	...
	public Building read(String filename) {
		...
		File xml = AndroidFileUtils.getFileFromPath(filename);
		/*
	...
	private class BuildingXmlHandler extends DefaultHandler {
		...
		public void startElement(String uri, String localName, String qName,
			...
			if (localName.equalsIgnoreCase("building")) {
				...
				if (path != null) {
					mAreaBuilder.readTextPoints(mParentDirectory + path);
				} else {
					throw new RuntimeException("required attribute path");
				}

			} else if (localName.equalsIgnoreCase("barcodes")) {
				// read path
				String path = null;
				for (int i = 0; i < attributes.getLength(); i++) {
				...
				if (path != null) {
					mAreaBuilder.readBarcodes(mParentDirectory + path);
				} else {
					throw new RuntimeException("required attribute path");
				}
