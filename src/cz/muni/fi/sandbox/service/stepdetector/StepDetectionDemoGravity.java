...
import cz.muni.fi.sandbox.service.stepdetector.StepDetectorGravity.StepDetectorGravityState;

public class StepDetectionDemoGravity extends Activity {
	...
	private class GraphView extends View implements SensorEventListener {
		...
		private StepDetectorGravity mStepDetector;
		...
		public GraphView(Context context) {
			...
			mStepDetector = new StepDetectorGravity(powerCutoff);
		...
		private void displayStepDetectorState(
				StepDetectorGravityState movingAverageStepDetectorState) {
			...
			maValues[0] = -movingAverageStepDetectorState.value[0];
			maValues[1] = -movingAverageStepDetectorState.value[1];
			maValues[2] = -movingAverageStepDetectorState.value[2];
			maValues[3] = movingAverageStepDetectorState.value[3];
		...
		public void processAccelerometerEvent(SensorEvent event) {
			...
			if (mBitmap != null) {
				mConvolution = (float) (mCC.process(Math.hypot(event.values[0], event.values[1])));
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mAccelerometer = getSensor(Sensor.TYPE_GRAVITY,
					"gravity sensor");
