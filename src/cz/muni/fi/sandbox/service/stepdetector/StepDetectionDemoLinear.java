...
public class StepDetectionDemoLinear extends Activity {
	...
	private class GraphView extends View implements SensorEventListener {
		...
		private MovingAverageStepDetectorLinear mStepDetector;
		...
		public GraphView(Context context) {
			...
			mStepDetector = new MovingAverageStepDetectorLinear(movingAverage1,
					movingAverage2, powerCutoff);
		...
		private void displayStepDetectorState(
				cz.muni.fi.sandbox.service.stepdetector.MovingAverageStepDetectorLinear.MovingAverageStepDetectorState movingAverageStepDetectorState) {
			...
			maValues[0] = -movingAverageStepDetectorState.values[0];
			maValues[1] = -movingAverageStepDetectorState.values[1];
			maValues[2] = -movingAverageStepDetectorState.values[2];
			maValues[3] = movingAverageStepDetectorState.values[3];

			stepDetected = movingAverageStepDetectorState.states[0];
			signalPowerCutoff = movingAverageStepDetectorState.states[1];
			...
			if (stepDetected) {
				...
				canvas.drawCircle(newX, mLastValues[1], 5, paint);
			}
			...
			paint.setColor(Color.BLACK);
			canvas.drawText("sensor rate: " + freqCounter.getRateF(), 0, 20,
					paint);
		...
		public void processAccelerometerEvent(SensorEvent event) {
			...
			if (mBitmap != null) {
				mConvolution = (float) (mCC.process(Math.hypot(event.values[1], event.values[2])));
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (currentapiVersion >= android.os.Build.VERSION_CODES.GINGERBREAD && isSensorSupported(this, Sensor.TYPE_LINEAR_ACCELERATION)) {
			mAccelerometer = getSensor(Sensor.TYPE_LINEAR_ACCELERATION,
					"linear acceleration sensor");
		} else if (isSensorSupported(this, Sensor.TYPE_ACCELEROMETER)) {
	...
	protected void onResume() {
		...
		if (mAccelerometer != null) mSensorManager.registerListener(mGraphView, mAccelerometer,
	...
	public static boolean isSensorSupported(Context context, int sensorType) {
